...
def call_openai(
    ...
    if latency == 0.0:
        latency = 0.001 # Fix for Windows time.time() resolution in fast mock tests
...
def compare_models(prompt: str) -> dict:
    ...
    cost = (len(gpt4o_answer.split()) / 0.75) / 1000 \
...
def run_assistant(
    ...
    while True:
        ...
        full_prompt = persona + " " + " ".join([m["content"] for m in history[:-2]]) + " " + user_msg
        cost_info = estimate_cost(full_prompt, reply, OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    table += "-" * 100 + "\n"
    for r in results:
        p = str(r.get("prompt", ""))[:40].replace("\n", " ")
        g = str(r.get("gpt4o_answer", ""))[:40].replace("\n", " ")
        m = str(r.get("mini_answer", ""))[:40].replace("\n", " ")
        gt = f'{r.get("gpt4o_time", 0):.2f}s'
        mt = f'{r.get("mini_time", 0):.2f}s'
        table += f"{p} | {g} | {m} | {gt} | {mt}\n"
    return table
